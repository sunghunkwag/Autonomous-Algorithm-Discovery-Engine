...
class DetectorParams:
    K_initial: int = 5               # CFG edit distance
    L_initial: int = 8               # active subseq length
    C_coverage: float = 0.55         # min coverage
    f_rarity: float = 0.001          # subseq rarity threshold
    N_repro: int = 4                 # reproducibility seeds
    require_both: bool = True
    min_loops: int = 1               # reject linear expansions
    min_scc: int = 1                 # require SCC presence
    allow_cfg_variants: int = 2      # reproducibility CFG variants
    max_cov_span: float = 0.30       # repro coverage stability
    max_loop_span: int = 5           # repro loop stability
...
class StrictStructuralDetector:
    ...
    def _K(self, gen: int) -> int:
        # You can anneal K if desired; keep stable for clean edition
        return max(3, int(self.p.K_initial))

    def _L(self, gen: int) -> int:
        return max(6, int(self.p.L_initial))

    def _anti_cheat(self, st: ExecutionState, code_len: int) -> Tuple[bool, str]:
        if st.error:
        ...
        if st.loops_count < self.p.min_loops:
    ...
    def _repro(self, genome: ProgramGenome, vm: VirtualMachine) -> Tuple[bool, str]:
        ...
        for i in range(self.p.N_repro):
            random.seed(i * 12345)
            inputs = [float(random.randint(0, 10)) for _ in range(8)]
    ...
    def evaluate(
        ...
        ok, msg = self._anti_cheat(st, len(genome.instructions))
        if not ok:
        ...
        if active:
            ...
            for seq in active:
                ...
                if freq < self.p.f_rarity:
                    ...
                    # Update archive counts on pass candidate (not only final success) to prevent replays
                    self.subseq_counts[seq] += 1
                    self.subseq_total += 1
...
class OmegaForgeV13:
    ...
    def step(self, writer: Optional[EvidenceWriter] = None) -> Tuple[int, int]:
        ...
        # Reproduce: keep elites + children
        random.shuffle(self.population)
        elites = self.population[: self.cfg.elite_keep]
        next_pop: List[ProgramGenome] = []
        for e in elites:
            # keep elite (clone to avoid accidental aliasing)
            kept = e.clone()
